...
| Script                 | Description                                                                                          |
|------------------------|------------------------------------------------------------------------------------------------------|
| `yarn clean:rm`        | Removes generated files such as `node_modules`, artifacts, and dependency lock files.               |
| `yarn clean:hardhat`   | Cleans up artifacts generated by Hardhat.                                                           |
| `yarn clean`           | Runs `clean:rm` to fully clean the project.                                                         |
| `yarn lint`            | Runs Hardhat's static checks to ensure the code is properly formatted and error-free.               |
| `yarn dev`             | Runs `lint` and starts Hardhat's compilation watcher for continuous development.                    |
| `yarn build`           | Compiles the project using Hardhat.                                                                 |
| `yarn test:sizer`      | Measures the size of deployed contracts using Hardhat.                                              |
| `yarn test`            | Runs the test suite with code coverage enabled.                                                     |
| `yarn ci`              | Cleans the project, installs dependencies, checks the code, and compiles it for CI pipelines.       |
| `yarn ci:deploy`       | Runs `ci` and then deploys the smart contracts to the configured networks.                          |
| `yarn ci:verify`       | Runs `ci` and then verifies the deployed smart contracts on the respective blockchains.             |
| `yarn chain:stress`    | Runs a stress test script on the local Hardhat network.                                             |
| `yarn deploy`          | Deploys the smart contracts to the configured network.                                              |
| `yarn deploy:<network>`| Deploys the smart contracts to the specified network (e.g., `ethereum`, `polygon`, `optimism`, etc.).|
| `yarn chain:devnet`    | Starts a local Hardhat development network.                                                         |
| `yarn verify`          | Verifies deployed contracts on the configured network.                                              |
| `yarn verify:<network>`| Verifies deployed contracts on the specified network (e.g., `ethereum`, `polygon`, `optimism`, etc.).|

Replace `<network>` with the appropriate blockchain network, such as `ethereum`, `polygon`, `bsc`, `optimism`, or others supported in the project.
